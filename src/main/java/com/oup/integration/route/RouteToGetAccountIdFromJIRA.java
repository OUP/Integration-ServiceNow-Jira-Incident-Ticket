...
public class RouteToGetAccountIdFromJIRA extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		.toD("{{jira.endpoint}}{{jira.userPath}}?query=${header.userEmail}{{jira.proxy}}&throwExceptionOnFailure=false")
		.convertBodyTo(String.class)
		...
		.when().simple("${header.CamelHttpResponseCode} == 200")
			...
			.otherwise()
				...
				.toD("{{jira.endpoint}}{{jira.userPath}}?query=${header.userNameWithContractor}{{jira.proxy}}&throwExceptionOnFailure=false")
				.convertBodyTo(String.class)
				.log("Jira response code:${header.CamelHttpResponseCode} and body: ${body}")
				.choice()
